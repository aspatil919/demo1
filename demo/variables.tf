variable "var_location" {
  type = string
}

variable "var_RG_name" {
  type = string
}

variable "var_Storage_name" {
  type = string
}

variable "var_account_tier" {
  type = string
}

variable "var_account_replication_type" {
  type = string
}

variable "var_storage_containername" {
  type = string
}

variable "var_DEVRG_name" {
  type = string
}

variable "var_DEVRG_storage_account_name" {
  type = string
}

variable "var_DEVRG_container_name" {
  type = string
}

variable "var_DEVRG_key" {
  type = string
}

variable "var_subscription_id" {
  type = string
}

variable "var_client_id" {
  type = string
}

variable "var_tenant_id" {
  type = string
}

variable "var_client_secret" {
  type = string
}











































































